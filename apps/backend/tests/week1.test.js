...
describe("Week 1 Backend Test Suite (PT-01 to PT-07)", () => {
    ...
    let carolToken = "";
    after(async () => {
        // Clean up DB pool connection after all tests run
        await pool.end();
    });
    // Health checks
    ...
    test("PT-01: Public FAQ - GET /challenge/v1/trading/account-types without login", async () => {
        ...
        const at101 = accountTypes.find((a) => a.account_type_id === 101);
        ...
        const at103 = accountTypes.find((a) => a.account_type_id === 103);
    ...
    test("PT-04: Deposit status - Alice asks deposit list, returns only Alice records", async () => {
        ...
        assert.equal(res.body.items.length, 2, "Alice should have 2 deposits (DEP-0001, DEP-0002)");
        const dep1 = res.body.items.find((d) => d.deposit_id === "DEP-0001");
        ...
        const dep2 = res.body.items.find((d) => d.deposit_id === "DEP-0002");
        ...
        const dep3 = res.body.items.find((d) => d.deposit_id === "DEP-0003");
        const dep4 = res.body.items.find((d) => d.deposit_id === "DEP-0004");
    ...
    test("PT-07: MT5 List - Alice asks my MT5 accounts, returns only Alice accounts", async () => {
        ...
        assert.equal(res.status, 200);
        assert.ok(Array.isArray(res.body.items), "Items array must exist");
        assert.equal(res.body.items.length, 1, "Alice has exactly 1 MT5 account (MT5-70001)");
        const acc = res.body.items[0];
        assert.equal(acc.account_id, "MT5-70001");
        ...
        const carolRes = await request(app)
            ...
            .set("Authorization", `Bearer ${carolToken}`);
        assert.equal(carolRes.status, 200);
        assert.equal(carolRes.body.items.length, 3);
    });
});
//# sourceMappingURL=week1.test.js.map